...
// Set current page for sidebar active highlighting
...
try {
    ...
    if (isset($_GET['user_id'])) {
        ...
        $stmt = $pdo->prepare("
            ...
            FROM users u
            LEFT JOIN sessions s ON (s.tutor_id = ? AND s.tutee_id = u.id) OR (s.tutee_id = ? AND s.tutor_id = u.id)
            LEFT JOIN units un ON s.unit_id = un.id
...
// Get user details for sidebar (with profile_picture)
$stmt = $pdo->prepare("
